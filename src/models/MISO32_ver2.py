...
class MISO_1(nn.Module):
    ...
    def en_make_layer(self,block_idx,in_channels, out_channels):
        ...
        if block_idx < 4:
            ...
            layers.append(Conv2d_(in_channels,out_channels,kernel_size=(3,3), stride=(1,2),padding=(1,1)))
        else:
            layers.append(Conv2d_(in_channels,out_channels,kernel_size=(3,3), stride=(1,2),padding=(1,0)))
    ...
    def forward(self,input):
        ...
        # # [B,T]
        # pdb.set_trace()
        # out = torch.squeeze(self.fc1(tcn_out.permute([0,2,1])))

        # return out
        
        ## MEL ##
        ## Consider : reference mic -> circular shift
        ...
        out = self.fc1(tcn_out.permute([0,2,1]))
        out = out[:,:,0]
